...
class Stats:
  ...
  def add(self, account_name, old_vesting_amount, new_vesting_amount):
    delta = Amount(old_vesting_amount) - Amount(new_vesting_amount)
    ...
    self.total_redeemed_vests += Decimal(str(delta).split(' ')[0])
  ...
  def get(self):
    return {
      ...
      "mode_redeemed_vests": self.mode_redeemed_vests,
